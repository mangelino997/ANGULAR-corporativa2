...
public readURL(event): void {
  if (event.target.files && event.target.files[0]) {
    ...
    reader.onload = e => this.indicativeImageForm.get('image').setValue(reader.result);
    reader.readAsDataURL(file);
  }
}
//Guarda en la Base de Datos
public save(){
  this.indicativeImageService.add(this.indicativeImageForm.get('image').value, this.indicativeImageForm.get('idTypeAnalysis').value.id).subscribe(res=>{
